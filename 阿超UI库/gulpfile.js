...
//模块
,mods = 'mytool,laytpl,laypage,laydate,jquery,layer,element,upload,slider,colorpicker,form,tree,transfer,table,carousel,rate,util,flow,layedit,code'
// ,mods = 'mytool'
//发行版本目录
,releaseDir = './release/zip/byui-v' + pkg.version
,release = releaseDir + '/byui'
...
,task = {
  ...
  ,alljs: function(ver){
    ...
    var src = [
      './src/**/{byui,all,'+ mods +'}.js'
      ,'!./src/**/mobile/*.js'
    ]
    ,dir = destDir(ver);
    
    return gulp.src(src).pipe(uglify())
      .pipe(concat('byui.all.js', {newLine: ''}))
      .pipe(header.apply(null, note))
    .pipe(gulp.dest('./'+ dir));
  }
