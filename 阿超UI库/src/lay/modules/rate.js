...
byui.define('jquery',function(exports){
  ...
  ,rate = {
    config: {}
    ,index: byui.rate ? (byui.rate.index + 10000) : 0
  ...
  //字符常量
  ,MOD_NAME = 'rate',ELEM_VIEW = 'byui-rate', ICON_RATE = 'byui-icon-rate', ICON_RATE_SOLID = 'byui-icon-rate-solid', ICON_RATE_HALF = 'byui-icon-rate-half'
  
  ,ICON_SOLID_HALF = 'byui-icon-rate-solid byui-icon-rate-half',  ICON_SOLID_RATE = 'byui-icon-rate-solid byui-icon-rate',  ICON_HALF_RATE = 'byui-icon-rate byui-icon-rate-half'
  ...
  Class.prototype.render = function(){
    ...
    //组件模板
    var temp = '<ul class="byui-rate" '+ (options.readonly ? 'readonly' : '') +'>';
    for(var i = 1;i <= options.length;i++){
      var item = '<li class="byui-inline"><i class="byui-icon '
      ...
      if(options.half){
        if(parseInt(options.value) !== options.value){
          if(i == Math.ceil(options.value)){
            temp = temp + '<li><i class="byui-icon byui-icon-rate-half" '+ style +'></i></li>';
          }else{
            temp = temp + item 
          } 
        }else{
    ...
    temp += '</ul>' + (options.text ? ('<span class="byui-inline">'+ options.value + '星') : '') + '</span>';
