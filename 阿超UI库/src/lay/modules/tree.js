...
byui.define('form', function(exports){
  "use strict";
  ...
  ,tree = {
    config: {}
    ,index: byui[MOD_NAME] ? (byui[MOD_NAME].index + 10000) : 0
  ...
  //字符常量
  ,SHOW = 'byui-show', HIDE = 'byui-hide', NONE = 'byui-none', DISABLED = 'byui-disabled'
  
  ,ELEM_VIEW = 'byui-tree', ELEM_SET = 'byui-tree-set', ICON_CLICK = 'byui-tree-iconClick'
  ,ICON_ADD = 'byui-icon-addition', ICON_SUB = 'byui-icon-subtraction', ELEM_ENTRY = 'byui-tree-entry', ELEM_MAIN = 'byui-tree-main', ELEM_TEXT = 'byui-tree-txt', ELEM_PACK = 'byui-tree-pack', ELEM_SPREAD = 'byui-tree-spread'
  ,ELEM_LINE_SHORT = 'byui-tree-setLineShort', ELEM_SHOW = 'byui-tree-showLine', ELEM_EXTEND = 'byui-tree-lineExtend'
  ...
  Class.prototype.reload = function(options){
    ...
    byui.each(options, function(key, item){
  ...
  Class.prototype.render = function(){
    ...
    var temp = $('<div class="byui-tree'+ (options.showCheckbox ? " byui-form" : "") + (options.showLine ? " byui-tree-line" : "") +'" lay-filter="LAY-tree-'+ that.index +'"></div>');
    that.tree(temp);
    ...
    if(options.showSearch){
      temp.prepend('<input type="text" class="byui-input byui-tree-search" placeholder="请输入关键字进行过滤">');
    };
    ...
    //插入组件结构
    that.elem = temp;
    that.elemNone = $('<div class="byui-tree-emptyText">'+ options.text.none +'</div>');
    othis.html(that.elem);

    if(that.elem.find('.byui-tree-set').length == 0){
    ...
    that.elem.find('.byui-tree-set').each(function(){
      var othis = $(this);
      //最外层
      if(!othis.parent('.byui-tree-pack')[0]){
        othis.addClass('byui-tree-setHide');
      };

      //没有下一个节点 上一层父级有延伸线
      if(!othis.next()[0] && othis.parents('.byui-tree-pack').eq(1).hasClass('byui-tree-lineExtend')){
        othis.addClass(ELEM_LINE_SHORT);
      };
      
      //没有下一个节点 外层最后一个
      if(!othis.next()[0] && !othis.parents('.byui-tree-set').eq(0).next()[0]){
        othis.addClass(ELEM_LINE_SHORT);
      };
    });
  ...
  Class.prototype.tree = function(elem, children){
    ...
    //遍历数据
    byui.each(data, function(index, item){
      var hasChild = item.children && item.children.length > 0
      ,packDiv = $('<div class="byui-tree-pack" '+ (item.spread ? 'style="display: block;"' : '') +'"></div>')
      ,entryDiv = $(['<div data-id="'+ item.id +'" class="byui-tree-set'+ (item.spread ? " byui-tree-spread" : "") + (item.checked ? " byui-tree-checkedFirst" : "") +'">'
        ,'<div '+ (options.drag && !item.fixed ? 'draggable="true"' : '') +' class="byui-tree-entry">'
          ,'<div class="byui-tree-main">'
            //箭头
            ,function(){
              if(options.showLine){
                if(hasChild){
                  return '<span class="byui-tree-iconClick byui-tree-icon"><i class="byui-icon '+ (item.spread ? "byui-icon-subtraction" : "byui-icon-addition") +'"></i></span>';
                }else{
                  return '<span class="byui-tree-iconClick"><i class="byui-icon byui-icon-file"></i></span>';
                };
              }else{
                return '<span class="byui-tree-iconClick"><i class="byui-tree-iconArrow '+ (hasChild ? "": HIDE) +'"></i></span>';
              };
            }()
      ...
      ,function(){
        ...
        var editIcon = {
          add: '<i class="byui-icon byui-icon-add-1"  data-type="add"></i>'
          ,update: '<i class="byui-icon byui-icon-edit" data-type="update"></i>'
          ,del: '<i class="byui-icon byui-icon-delete" data-type="del"></i>'
        }, arr = ['<div class="byui-btn-group byui-tree-btnGroup">'];
        ...
        if(typeof options.edit === 'object'){
          byui.each(options.edit, function(i, val){
      ...
      if(entryDiv.prev('.'+ELEM_SET)[0]){
        entryDiv.prev().children('.byui-tree-pack').addClass('byui-tree-showLine');
      };
      
      //若无子节点，则父节点加延伸线
      if(!hasChild){
        entryDiv.parent('.byui-tree-pack').addClass('byui-tree-lineExtend');
      };
  ...
  Class.prototype.spread = function(elem, item){
    ...
    touchOpen.on('click', function(e){
      ...
      ,iconClick = touchOpen.children('.byui-icon')[0] ? touchOpen.children('.byui-icon') : touchOpen.find('.byui-tree-icon').children('.byui-icon');
      ...
      if(!packCont[0]){
        ...
        if(elem.hasClass(ELEM_SPREAD)){
          ...
          if(options.accordion){
            ...
            sibls.find('.byui-tree-icon').children('.byui-icon').removeClass(ICON_SUB).addClass(ICON_ADD);
          };
        };
      };
    });
  ...
  Class.prototype.operate = function(elem, item){
    ...
    entry.children('.byui-tree-btnGroup').on('click', '.byui-icon', function(e){
      byui.stope(e);  //阻止节点操作
      ...
      if(type == 'add'){
        ...
        if(!packCont[0]){
          //若开启连接线，更改图标样式
          if(options.showLine){
            elemMain.find('.'+ICON_CLICK).addClass('byui-tree-icon');
            elemMain.find('.'+ICON_CLICK).children('.byui-icon').addClass(ICON_ADD).removeClass('byui-icon-file');
          //若未开启连接线，显示箭头
          }else{
            elemMain.find('.byui-tree-iconArrow').removeClass(HIDE);
          };
          //节点添加子节点容器
          elem.append('<div class="byui-tree-pack"></div>');
        };
        ...
        if(options.showLine){
          ...
          if(!packCont[0]){
            ...
            byui.each(siblings, function(index, i){
      ...
      //修改
        ...
        //添加输入框，覆盖在文字上方
        elemMain.append('<input type="text" class="byui-tree-editInput">');
        //获取焦点
        elemMain.children('.byui-tree-editInput').val(text).focus();
        //嵌入文字移除输入框
        ...
        //失去焦点
        elemMain.children('.byui-tree-editInput').blur(function(){
          getVal($(this));
        });
        //回车
        elemMain.children('.byui-tree-editInput').on('keydown', function(e){
          if(e.keyCode === 13){
            e.preventDefault();
            getVal($(this));
          };
      ...
      //删除
        ...
        if(elem.siblings('.'+ELEM_SET).children('.'+ELEM_ENTRY)[0]){
          ...
          //若开启了连接线
          if(options.showLine){
            prevDiv.find('.'+ICON_CLICK).removeClass('byui-tree-icon');
            prevDiv.find('.'+ICON_CLICK).children('.byui-icon').removeClass(ICON_SUB).addClass('byui-icon-file');
            //父节点所在层添加延伸线
          ...
          //父节点隐藏箭头
            prevDiv.find('.byui-tree-iconArrow').addClass(HIDE);
          };
          //移除展开属性
  ...
  Class.prototype.drag = function(){
    ...
    that.elem.on('dragend', '.'+ELEM_ENTRY, function(e){
      ...
      var setDel = function(parentSet){
        ...
        //若为最外层节点，不做下列操作
        if(!isTree){
          setParent.removeClass('byui-tree-setHide');
          return;
        };
        ...
        if(num == 1){
          if(options.showLine){
            parentSet.find('.'+ICON_CLICK).removeClass('byui-tree-icon');
            parentSet.find('.'+ICON_CLICK).children('.byui-icon').removeClass(ICON_SUB).addClass('byui-icon-file');
            warpPack.addClass(ELEM_EXTEND);
            warpPack.children('.'+ELEM_SET).children('.'+ELEM_PACK).each(function(){
              $(this).children('.'+ELEM_SET).last().addClass(ELEM_LINE_SHORT);
            });
          }else{
            parentSet.find('.byui-tree-iconArrow').addClass(HIDE);
          };
          ...
          if(options.showLine){
            //遍历兄弟节点，判断兄弟节点是否有子节点
            var number = 1;
            byui.each(siblingOld, function(index, i){
              if(!$(i).children('.'+ELEM_PACK)[0]){
                number = 0;
              };
            });
        ...
        if($(this).height() != 0){
          ...
          if((disY > thisTop) && (disY < thisTop + 15)){
            ...
            if(!$(this).children('.'+ELEM_PACK)[0]){
              if(options.showLine){
                $(this).find('.'+ICON_CLICK).eq(0).addClass('byui-tree-icon');
                $(this).find('.'+ICON_CLICK).eq(0).children('.byui-icon').addClass(ICON_ADD).removeClass('byui-icon-file');
              }else{
                $(this).find(".byui-tree-iconArrow").removeClass(HIDE);
              };
              $(this).append('<div class="byui-tree-pack"></div>');
            };
            //插入元素
            ...
            if(options.showLine){
              ...
              if(children.length == 1){
                ...
                if(ss == 1){
                  ...
                  parentPack.removeClass(ELEM_EXTEND);
                  //同层节点最后一个去除连接线
                  parentPack.children('.'+ELEM_SET).last().children('.'+ELEM_PACK).children('.'+ELEM_SET).last().addClass(ELEM_LINE_SHORT).removeClass('byui-tree-setHide');
                }else{
                  $(this).children('.'+ELEM_PACK).children('.'+ELEM_SET).addClass(ELEM_LINE_SHORT).removeClass('byui-tree-setHide');
                };
              }else{
                //若原子节点含有延伸线
                ...
                //清除之前状态
                  setParent.removeClass('byui-tree-setLineShort byui-tree-setHide');
                  //若添加节点无子节点
                  if(!setParent.children('.'+ELEM_PACK)[0]){
                    //兄弟节点子节点添加延伸线
          ...
          //若位于元素上方
            ...
            if(options.showLine){
              ...
              if(!setFirst[0]){
                //隐藏前置连接线
                setParent.addClass('byui-tree-setHide');
  ...
  Class.prototype.events = function(){
    var that = this
    ,options = that.config
    ,checkWarp = that.elem.find('.byui-tree-checkedFirst');
    
     //初始选中
    byui.each(checkWarp, function(i, item){
      $(item).children('.'+ELEM_ENTRY).find('input[name="layuiTreeCheck"]+').trigger('click');
    });
    
    //搜索
    that.elem.find('.byui-tree-search').on('keyup', function(){
      var input = $(this)
      ,val = input.val()
      ,pack = input.nextAll()
      ,arr = [];
      ...
      pack.find('.'+ ELEM_TEXT).each(function(){
        ...
        if($(this).html().indexOf(val) != -1){
          ...
          var select = function(div){
            div.addClass('byui-tree-searchShow');
            //向上父节点渲染
      ...
      pack.find('.'+ELEM_ENTRY).each(function(){
        var parent = $(this).parent('.'+ELEM_SET);
        if(!parent.hasClass('byui-tree-searchShow')){
          parent.addClass(HIDE);
        };
      });
      if(pack.find('.byui-tree-searchShow').length == 0){
        that.elem.append(that.elemNone);
      };
    ...
    //还原搜索初始状态
    that.elem.find('.byui-tree-search').on('keydown', function(){
      $(this).nextAll().find('.'+ELEM_ENTRY).each(function(){
        var parent = $(this).parent('.'+ELEM_SET);
        parent.removeClass('byui-tree-searchShow '+ HIDE);
      });
      if($('.byui-tree-emptyText')[0]) $('.byui-tree-emptyText').remove();
    });
  };
  ...
  Class.prototype.getChecked = function(){
    ...
    //遍历节点找到选中索引
    that.elem.find('.byui-form-checked').each(function(){
    ...
    var eachNodes = function(data, checkNode){
      byui.each(data, function(index, item){
        byui.each(checkId, function(index2, item2){
          if(item.id == item2){
