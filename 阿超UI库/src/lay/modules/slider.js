...
byui.define('jquery', function(exports){
  ...
  ,slider = {
    config: {}
    ,index: byui.slider ? (byui.slider.index + 10000) : 0
  ...
  //字符常量
  ,MOD_NAME = 'slider', DISABLED = 'byui-disabled', ELEM_VIEW = 'byui-slider', SLIDER_BAR = 'byui-slider-bar', SLIDER_WRAP = 'byui-slider-wrap', SLIDER_WRAP_BTN = 'byui-slider-wrap-btn', SLIDER_TIPS = 'byui-slider-tips', SLIDER_INPUT = 'byui-slider-input', SLIDER_INPUT_TXT = 'byui-slider-input-txt', SLIDER_INPUT_BTN = 'byui-slider-input-btn', ELEM_HOVER = 'byui-slider-hover'
  ...
  Class.prototype.render = function(){
    ...
    //滑块
    var temp = '<div class="byui-slider '+ (options.type === 'vertical' ? 'byui-slider-vertical' : '') +'">'+ (options.tips ? '<div class="byui-slider-tips"></div>' : '') + 
    '<div class="byui-slider-bar" style="background:'+ theme +'; '+ (options.type === 'vertical' ? 'height' : 'width') +':'+ scale +';'+ (options.type === 'vertical' ? 'bottom' : 'left') +':'+ (scaleFir || 0) +';"></div><div class="byui-slider-wrap" style="'+ (options.type === 'vertical' ? 'bottom' : 'left') +':'+ (scaleFir || scale) +';">' +
    '<div class="byui-slider-wrap-btn" style="border: 2px solid '+ theme +';"></div></div>'+ (options.range ? '<div class="byui-slider-wrap" style="'+ (options.type === 'vertical' ? 'bottom' : 'left') +':'+ scaleSec +';"><div class="byui-slider-wrap-btn" style="border: 2px solid '+ theme +';"></div></div>' : '') +'</div>';
    ...
    if(options.showstep){
      ...
      for(var i = 1; i < number + 1; i++) {
        ...
        if(step < 100){
          item += '<div class="byui-slider-step" style="'+ (options.type === 'vertical' ? 'bottom' : 'left') +':'+ step +'%"></div>'
        }
      };
    ...
    if(options.input && !options.range){
      var elemInput = $('<div class="byui-slider-input byui-input"><div class="byui-slider-input-txt"><input type="text" class="byui-input"></div><div class="byui-slider-input-btn"><i class="byui-icon byui-icon-up"></i><i class="byui-icon byui-icon-down"></i></div></div>');
  ...
  Class.prototype.slide = function(setValue, value, i){
    ...
    //拖拽元素
    ,elemMove = $(['<div class="byui-auxiliar-moving" id="LAY-slider-moving"></div'].join(''))
