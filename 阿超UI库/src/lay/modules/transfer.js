/**
 ...
 @Name：byui.transfer 穿梭框
 @Author：贤心
 @License：MIT

 */

byui.define(['laytpl', 'form'], function(exports){
  "use strict";
  ...
  //字符常量
  ,ELEM = 'byui-transfer', HIDE = 'byui-hide', DISABLED = 'byui-btn-disabled', NONE = 'byui-none'
  ,ELEM_BOX = 'byui-transfer-box', ELEM_HEADER = 'byui-transfer-header', ELEM_SEARCH = 'byui-transfer-search', ELEM_ACTIVE = 'byui-transfer-active', ELEM_DATA = 'byui-transfer-data'
  
  //穿梭框模板
  ,TPL_BOX = function(obj){
    obj = obj || {};
    return ['<div class="byui-transfer-box" data-index="'+ obj.index +'">'
      ,'<div class="byui-transfer-header">'
      ...
      ,'<div class="byui-transfer-search">'
        ,'<i class="byui-icon byui-icon-search"></i>'
        ,'<input type="input" class="byui-input" placeholder="关键词搜索">'
      ,'</div>'
      ,'{{# } }}'
      ,'<ul class="byui-transfer-data"></ul>'
    ,'</div>'].join('');
  }
  
  //主模板
  ,TPL_MAIN = ['<div class="byui-transfer byui-form byui-border-box" lay-filter="LAY-transfer-{{ d.index }}">'
    ,TPL_BOX({
      index: 0
      ,checkAllName: 'layTransferLeftCheckAll'
    })
    ,'<div class="byui-transfer-active">'
      ,'<button type="button" class="byui-btn byui-btn-sm byui-btn-primary byui-btn-disabled" data-index="0">'
        ,'<i class="byui-icon byui-icon-next"></i>'
      ,'</button>'
      ,'<button type="button" class="byui-btn byui-btn-sm byui-btn-primary byui-btn-disabled" data-index="1">'
        ,'<i class="byui-icon byui-icon-prev"></i>'
      ,'</button>'
    ,'</div>'
    ,TPL_BOX({
      index: 1
  ...
  Class.prototype.render = function(){
    ...
    that.layBtn = thisElem.find('.'+ ELEM_ACTIVE + ' .byui-btn');
  ...
  Class.prototype.noneView = function(thisDataElem, text){
    var createNoneElem = $('<p class="byui-none">'+ (text || '') +'</p>');
  ...
  Class.prototype.parseData = function(callback){
    ...
    byui.each(options.data, function(index, item){
      //解析格式
      ...
      byui.each(options.value, function(index2, item2){
        if(item2 == item.value){
          item.selected = true;
        }
      });
  ...
  Class.prototype.getData = function(value){
    ...
    ,options = that.config
    ,selectedData = [];
    byui.each(value || options.value, function(index, item){
      byui.each(options.data, function(index2, item2){
