...
byui.define('jquery', function(exports){
  ...
  ,colorpicker = {
    config: {}
    ,index: byui.colorpicker ? (byui.colorpicker.index + 10000) : 0
    ...
    ,on: function(events, callback){
      return byui.onevent.call(this, 'colorpicker', events, callback);
    }
  }
  ...
  //字符常量
  ,MOD_NAME = 'colorpicker', SHOW = 'byui-show', THIS = 'byui-this', ELEM = 'byui-colorpicker'
  
  ,ELEM_MAIN = '.byui-colorpicker-main', ICON_PICKER_DOWN = 'byui-icon-down', ICON_PICKER_CLOSE = 'byui-icon-close'
  ,PICKER_TRIG_SPAN = 'byui-colorpicker-trigger-span', PICKER_TRIG_I = 'byui-colorpicker-trigger-i', PICKER_SIDE = 'byui-colorpicker-side', PICKER_SIDE_SLIDER = 'byui-colorpicker-side-slider'
  ,PICKER_BASIS = 'byui-colorpicker-basis', PICKER_ALPHA_BG = 'byui-colorpicker-alpha-bgcolor', PICKER_ALPHA_SLIDER = 'byui-colorpicker-alpha-slider', PICKER_BASIS_CUR = 'byui-colorpicker-basis-cursor', PICKER_INPUT = 'byui-colorpicker-main-input'
  ...
  Class.prototype.render = function(){
    ...
    //颜色选择框对象
    ,elemColorBox = $(['<div class="byui-unselect byui-colorpicker">'
      ,'<span '+ (options.format == 'rgb' && options.alpha
          ? 'class="byui-colorpicker-trigger-bgcolor"'
        : '') +'>'
        ,'<span class="byui-colorpicker-trigger-span" '
          ...
          ,'<i class="byui-icon byui-colorpicker-trigger-i '+ (options.color 
            ? ICON_PICKER_DOWN 
          : ICON_PICKER_CLOSE) +'"></i>'
        ,'</span>'
      ,'</span>'
    ...
    options.size && elemColorBox.addClass('byui-colorpicker-'+ options.size); //初始化颜色选择框尺寸
    
    //插入颜色选择框
    othis.addClass('byui-inline').html(
  ...
  Class.prototype.renderPicker = function(){
    ...
    //颜色选择器对象
    ,elemPicker = that.elemPicker = $(['<div id="byui-colorpicker'+ that.index +'" data-index="'+ that.index +'" class="byui-anim byui-anim-upbit byui-colorpicker-main">'
      //颜色面板
      ,'<div class="byui-colorpicker-main-wrapper">'
        ,'<div class="byui-colorpicker-basis">'
          ,'<div class="byui-colorpicker-basis-white"></div>'
          ,'<div class="byui-colorpicker-basis-black"></div>'
          ,'<div class="byui-colorpicker-basis-cursor"></div>'
        ,'</div>'
        ,'<div class="byui-colorpicker-side">'
          ,'<div class="byui-colorpicker-side-slider"></div>'
        ,'</div>'
      ,'</div>'
      
      //透明度条块
      ,'<div class="byui-colorpicker-main-alpha '+ (options.alpha ? SHOW : '') +'">'
        ,'<div class="byui-colorpicker-alpha-bgcolor">'
          ,'<div class="byui-colorpicker-alpha-slider"></div>'
        ,'</div>'
      ,'</div>'
      ...
      ,function(){
        if(options.predefine){
          var list = ['<div class="byui-colorpicker-main-pre">'];
          byui.each(options.colors, function(i, v){
            list.push(['<div class="byui-colorpicker-pre'+ ((v.match(/[0-9]{1,3}/g) || []).length > 3 
              ? ' byui-colorpicker-pre-isalpha' 
            : '') +'">'
      ...
      //底部表单元素区域
      ,'<div class="byui-colorpicker-main-input">'
        ,'<div class="byui-inline">'
          ,'<input type="text" class="byui-input">'
        ,'</div>'
        ,'<div class="byui-btn-container">'
          ,'<button class="byui-btn byui-btn-primary byui-btn-sm" colorpicker-events="clear">清空</button>'
          ,'<button class="byui-btn byui-btn-sm" colorpicker-events="confirm">确定</button>'
        ,'</div'
      ,'</div>'
    ,'</div>'].join(''))
  ...
  Class.prototype.removePicker = function(index){
    var that = this
    ,options = that.config;
    $('#byui-colorpicker'+ (index || that.index)).remove();
    return that;
  };
  ...
  Class.prototype.side = function(){
    ...
    ,pre = that.elemPicker.find('.byui-colorpicker-pre').children('div')
    ...
    //拖拽元素
    ,elemMove = $(['<div class="byui-auxiliar-moving" id="LAY-colorpicker-moving"></div'].join(''))
    ...
    pre.each(function(){
      $(this).on('click', function(){
        $(this).parent('.byui-colorpicker-pre').addClass('selected').siblings().removeClass('selected');
