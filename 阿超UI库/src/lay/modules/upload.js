...
byui.define('layer' , function(exports){
  ...
  //字符常量
  ,MOD_NAME = 'upload', ELEM = '.byui-upload', THIS = 'byui-this', SHOW = 'byui-show', HIDE = 'byui-hide', DISABLED = 'byui-disabled'
  
  ,ELEM_FILE = 'byui-upload-file', ELEM_FORM = 'byui-upload-form', ELEM_IFRAME = 'byui-upload-iframe', ELEM_CHOOSE = 'byui-upload-choose', ELEM_DRAG = 'byui-upload-drag'
  ...
  Class.prototype.file = function(){
    ...
    if(device.ie && device.ie < 10){
      options.elem.wrap('<div class="byui-upload-wrap"></div>');
    }
  ...
  Class.prototype.initIE = function(){
    ...
    if(!options.elem.next().hasClass(ELEM_FORM)){
      ...
      options.elem.next('.'+ ELEM_FORM).append(function(){
        var arr = [];
        byui.each(options.data, function(key, value){
  ...
  Class.prototype.preview = function(callback){
    ...
    if(window.FileReader){
      byui.each(that.chooseFiles, function(index, file){
  ...
  Class.prototype.upload = function(files, type){
    ...
    ,ajaxSend = function(){
      ...
      byui.each(items, function(index, file){
    ...
    ,check ,value = function(){
      var arr = [];
      byui.each(files || that.chooseFiles, function(i, item){
        arr.push(item.name);
      });
      return arr;
    }()
    ...
    ,args = {
      ...
      ,pushFile: function(){
        ...
        byui.each(that.chooseFiles, function(index, item){
    ...
    switch(options.accept){
      ...
      break;
      default: //图片文件
        byui.each(value, function(i, item){
  ...
  Class.prototype.events = function(){
    ...
    ,setChooseFile = function(files){
      that.chooseFiles = {};
      byui.each(files, function(i, item){
    ...
    ,setChooseText = function(files, filename){
      ...
      elemFile.after('<span class="byui-inline '+ ELEM_CHOOSE +'">'+ value +'</span>');
    };
