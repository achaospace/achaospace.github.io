...
baiyun.define('form', function(exports){
  "use strict";
  ...
  ,tree = {
    config: {}
    ,index: baiyun[MOD_NAME] ? (baiyun[MOD_NAME].index + 10000) : 0
  ...
  //字符常量
  ,SHOW = 'baiyun-show', HIDE = 'baiyun-hide', NONE = 'baiyun-none', DISABLED = 'baiyun-disabled'
  
  ,ELEM_VIEW = 'baiyun-tree', ELEM_SET = 'baiyun-tree-set', ICON_CLICK = 'baiyun-tree-iconClick'
  ,ICON_ADD = 'baiyun-icon-addition', ICON_SUB = 'baiyun-icon-subtraction', ELEM_ENTRY = 'baiyun-tree-entry', ELEM_MAIN = 'baiyun-tree-main', ELEM_TEXT = 'baiyun-tree-txt', ELEM_PACK = 'baiyun-tree-pack', ELEM_SPREAD = 'baiyun-tree-spread'
  ,ELEM_LINE_SHORT = 'baiyun-tree-setLineShort', ELEM_SHOW = 'baiyun-tree-showLine', ELEM_EXTEND = 'baiyun-tree-lineExtend'
  ...
  Class.prototype.reload = function(options){
    ...
    baiyun.each(options, function(key, item){
  ...
  Class.prototype.render = function(){
    ...
    var temp = $('<div class="baiyun-tree'+ (options.showCheckbox ? " baiyun-form" : "") + (options.showLine ? " baiyun-tree-line" : "") +'" lay-filter="LAY-tree-'+ that.index +'"></div>');
    that.tree(temp);
    ...
    if(options.showSearch){
      temp.prepend('<input type="text" class="baiyun-input baiyun-tree-search" placeholder="请输入关键字进行过滤">');
    };
    ...
    //插入组件结构
    that.elem = temp;
    that.elemNone = $('<div class="baiyun-tree-emptyText">'+ options.text.none +'</div>');
    othis.html(that.elem);

    if(that.elem.find('.baiyun-tree-set').length == 0){
    ...
    that.elem.find('.baiyun-tree-set').each(function(){
      var othis = $(this);
      //最外层
      if(!othis.parent('.baiyun-tree-pack')[0]){
        othis.addClass('baiyun-tree-setHide');
      };

      //没有下一个节点 上一层父级有延伸线
      if(!othis.next()[0] && othis.parents('.baiyun-tree-pack').eq(1).hasClass('baiyun-tree-lineExtend')){
        othis.addClass(ELEM_LINE_SHORT);
      };
      
      //没有下一个节点 外层最后一个
      if(!othis.next()[0] && !othis.parents('.baiyun-tree-set').eq(0).next()[0]){
        othis.addClass(ELEM_LINE_SHORT);
      };
    });
  ...
  Class.prototype.tree = function(elem, children){
    ...
    //遍历数据
    baiyun.each(data, function(index, item){
      var hasChild = item.children && item.children.length > 0
      ,packDiv = $('<div class="baiyun-tree-pack" '+ (item.spread ? 'style="display: block;"' : '') +'"></div>')
      ,entryDiv = $(['<div data-id="'+ item.id +'" class="baiyun-tree-set'+ (item.spread ? " baiyun-tree-spread" : "") + (item.checked ? " baiyun-tree-checkedFirst" : "") +'">'
        ,'<div '+ (options.drag && !item.fixed ? 'draggable="true"' : '') +' class="baiyun-tree-entry">'
          ,'<div class="baiyun-tree-main">'
            //箭头
            ,function(){
              if(options.showLine){
                if(hasChild){
                  return '<span class="baiyun-tree-iconClick baiyun-tree-icon"><i class="baiyun-icon '+ (item.spread ? "baiyun-icon-subtraction" : "baiyun-icon-addition") +'"></i></span>';
                }else{
                  return '<span class="baiyun-tree-iconClick"><i class="baiyun-icon baiyun-icon-file"></i></span>';
                };
              }else{
                return '<span class="baiyun-tree-iconClick"><i class="baiyun-tree-iconArrow '+ (hasChild ? "": HIDE) +'"></i></span>';
              };
            }()
      ...
      ,function(){
        ...
        var editIcon = {
          add: '<i class="baiyun-icon baiyun-icon-add-1"  data-type="add"></i>'
          ,update: '<i class="baiyun-icon baiyun-icon-edit" data-type="update"></i>'
          ,del: '<i class="baiyun-icon baiyun-icon-delete" data-type="del"></i>'
        }, arr = ['<div class="baiyun-btn-group baiyun-tree-btnGroup">'];
        ...
        if(typeof options.edit === 'object'){
          baiyun.each(options.edit, function(i, val){
      ...
      if(entryDiv.prev('.'+ELEM_SET)[0]){
        entryDiv.prev().children('.baiyun-tree-pack').addClass('baiyun-tree-showLine');
      };
      
      //若无子节点，则父节点加延伸线
      if(!hasChild){
        entryDiv.parent('.baiyun-tree-pack').addClass('baiyun-tree-lineExtend');
      };
  ...
  Class.prototype.spread = function(elem, item){
    ...
    touchOpen.on('click', function(e){
      ...
      ,iconClick = touchOpen.children('.baiyun-icon')[0] ? touchOpen.children('.baiyun-icon') : touchOpen.find('.baiyun-tree-icon').children('.baiyun-icon');
      ...
      if(!packCont[0]){
        ...
        if(elem.hasClass(ELEM_SPREAD)){
          ...
          if(options.accordion){
            ...
            sibls.find('.baiyun-tree-icon').children('.baiyun-icon').removeClass(ICON_SUB).addClass(ICON_ADD);
          };
        };
      };
    });
  ...
  Class.prototype.operate = function(elem, item){
    ...
    entry.children('.baiyun-tree-btnGroup').on('click', '.baiyun-icon', function(e){
      baiyun.stope(e);  //阻止节点操作
      ...
      if(type == 'add'){
        ...
        if(!packCont[0]){
          //若开启连接线，更改图标样式
          if(options.showLine){
            elemMain.find('.'+ICON_CLICK).addClass('baiyun-tree-icon');
            elemMain.find('.'+ICON_CLICK).children('.baiyun-icon').addClass(ICON_ADD).removeClass('baiyun-icon-file');
          //若未开启连接线，显示箭头
          }else{
            elemMain.find('.baiyun-tree-iconArrow').removeClass(HIDE);
          };
          //节点添加子节点容器
          elem.append('<div class="baiyun-tree-pack"></div>');
        };
        ...
        if(options.showLine){
          ...
          if(!packCont[0]){
            ...
            baiyun.each(siblings, function(index, i){
      ...
      //修改
        ...
        //添加输入框，覆盖在文字上方
        elemMain.append('<input type="text" class="baiyun-tree-editInput">');
        //获取焦点
        elemMain.children('.baiyun-tree-editInput').val(text).focus();
        //嵌入文字移除输入框
        ...
        //失去焦点
        elemMain.children('.baiyun-tree-editInput').blur(function(){
          getVal($(this));
        });
        //回车
        elemMain.children('.baiyun-tree-editInput').on('keydown', function(e){
          if(e.keyCode === 13){
            e.preventDefault();
            getVal($(this));
          };
      ...
      //删除
        ...
        if(elem.siblings('.'+ELEM_SET).children('.'+ELEM_ENTRY)[0]){
          ...
          //若开启了连接线
          if(options.showLine){
            prevDiv.find('.'+ICON_CLICK).removeClass('baiyun-tree-icon');
            prevDiv.find('.'+ICON_CLICK).children('.baiyun-icon').removeClass(ICON_SUB).addClass('baiyun-icon-file');
            //父节点所在层添加延伸线
          ...
          //父节点隐藏箭头
            prevDiv.find('.baiyun-tree-iconArrow').addClass(HIDE);
          };
          //移除展开属性
  ...
  Class.prototype.drag = function(){
    ...
    that.elem.on('dragend', '.'+ELEM_ENTRY, function(e){
      ...
      var setDel = function(parentSet){
        ...
        //若为最外层节点，不做下列操作
        if(!isTree){
          setParent.removeClass('baiyun-tree-setHide');
          return;
        };
        ...
        if(num == 1){
          if(options.showLine){
            parentSet.find('.'+ICON_CLICK).removeClass('baiyun-tree-icon');
            parentSet.find('.'+ICON_CLICK).children('.baiyun-icon').removeClass(ICON_SUB).addClass('baiyun-icon-file');
            warpPack.addClass(ELEM_EXTEND);
            warpPack.children('.'+ELEM_SET).children('.'+ELEM_PACK).each(function(){
              $(this).children('.'+ELEM_SET).last().addClass(ELEM_LINE_SHORT);
            });
          }else{
            parentSet.find('.baiyun-tree-iconArrow').addClass(HIDE);
          };
          ...
          if(options.showLine){
            //遍历兄弟节点，判断兄弟节点是否有子节点
            var number = 1;
            baiyun.each(siblingOld, function(index, i){
              if(!$(i).children('.'+ELEM_PACK)[0]){
                number = 0;
              };
            });
        ...
        if($(this).height() != 0){
          ...
          if((disY > thisTop) && (disY < thisTop + 15)){
            ...
            if(!$(this).children('.'+ELEM_PACK)[0]){
              if(options.showLine){
                $(this).find('.'+ICON_CLICK).eq(0).addClass('baiyun-tree-icon');
                $(this).find('.'+ICON_CLICK).eq(0).children('.baiyun-icon').addClass(ICON_ADD).removeClass('baiyun-icon-file');
              }else{
                $(this).find(".baiyun-tree-iconArrow").removeClass(HIDE);
              };
              $(this).append('<div class="baiyun-tree-pack"></div>');
            };
            //插入元素
            ...
            if(options.showLine){
              ...
              if(children.length == 1){
                ...
                if(ss == 1){
                  ...
                  parentPack.removeClass(ELEM_EXTEND);
                  //同层节点最后一个去除连接线
                  parentPack.children('.'+ELEM_SET).last().children('.'+ELEM_PACK).children('.'+ELEM_SET).last().addClass(ELEM_LINE_SHORT).removeClass('baiyun-tree-setHide');
                }else{
                  $(this).children('.'+ELEM_PACK).children('.'+ELEM_SET).addClass(ELEM_LINE_SHORT).removeClass('baiyun-tree-setHide');
                };
              }else{
                //若原子节点含有延伸线
                ...
                //清除之前状态
                  setParent.removeClass('baiyun-tree-setLineShort baiyun-tree-setHide');
                  //若添加节点无子节点
                  if(!setParent.children('.'+ELEM_PACK)[0]){
                    //兄弟节点子节点添加延伸线
          ...
          //若位于元素上方
            ...
            if(options.showLine){
              ...
              if(!setFirst[0]){
                //隐藏前置连接线
                setParent.addClass('baiyun-tree-setHide');
  ...
  Class.prototype.events = function(){
    var that = this
    ,options = that.config
    ,checkWarp = that.elem.find('.baiyun-tree-checkedFirst');
    
     //初始选中
    baiyun.each(checkWarp, function(i, item){
      $(item).children('.'+ELEM_ENTRY).find('input[name="layuiTreeCheck"]+').trigger('click');
    });
    
    //搜索
    that.elem.find('.baiyun-tree-search').on('keyup', function(){
      var input = $(this)
      ,val = input.val()
      ,pack = input.nextAll()
      ,arr = [];
      ...
      pack.find('.'+ ELEM_TEXT).each(function(){
        ...
        if($(this).html().indexOf(val) != -1){
          ...
          var select = function(div){
            div.addClass('baiyun-tree-searchShow');
            //向上父节点渲染
      ...
      pack.find('.'+ELEM_ENTRY).each(function(){
        var parent = $(this).parent('.'+ELEM_SET);
        if(!parent.hasClass('baiyun-tree-searchShow')){
          parent.addClass(HIDE);
        };
      });
      if(pack.find('.baiyun-tree-searchShow').length == 0){
        that.elem.append(that.elemNone);
      };
    ...
    //还原搜索初始状态
    that.elem.find('.baiyun-tree-search').on('keydown', function(){
      $(this).nextAll().find('.'+ELEM_ENTRY).each(function(){
        var parent = $(this).parent('.'+ELEM_SET);
        parent.removeClass('baiyun-tree-searchShow '+ HIDE);
      });
      if($('.baiyun-tree-emptyText')[0]) $('.baiyun-tree-emptyText').remove();
    });
  };
  ...
  Class.prototype.getChecked = function(){
    ...
    //遍历节点找到选中索引
    that.elem.find('.baiyun-form-checked').each(function(){
    ...
    var eachNodes = function(data, checkNode){
      baiyun.each(data, function(index, item){
        baiyun.each(checkId, function(index2, item2){
          if(item.id == item2){
