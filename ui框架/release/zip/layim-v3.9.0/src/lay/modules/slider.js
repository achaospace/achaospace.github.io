/**
 ...
 @Title: baiyun.slider 滑块
 @Author: star1029
 @License：MIT

 */

baiyun.define('jquery', function(exports){
  "use strict";
  var $ = baiyun.jquery
  ...
  ,slider = {
    config: {}
    ,index: baiyun.slider ? (baiyun.slider.index + 10000) : 0
  ...
  //字符常量
  ,MOD_NAME = 'slider', DISABLED = 'baiyun-disabled', ELEM_VIEW = 'baiyun-slider', SLIDER_BAR = 'baiyun-slider-bar', SLIDER_WRAP = 'baiyun-slider-wrap', SLIDER_WRAP_BTN = 'baiyun-slider-wrap-btn', SLIDER_TIPS = 'baiyun-slider-tips', SLIDER_INPUT = 'baiyun-slider-input', SLIDER_INPUT_TXT = 'baiyun-slider-input-txt', SLIDER_INPUT_BTN = 'baiyun-slider-input-btn', ELEM_HOVER = 'baiyun-slider-hover'
  ...
  Class.prototype.render = function(){
    ...
    //滑块
    var temp = '<div class="baiyun-slider '+ (options.type === 'vertical' ? 'baiyun-slider-vertical' : '') +'">'+ (options.tips ? '<div class="baiyun-slider-tips"></div>' : '') + 
    '<div class="baiyun-slider-bar" style="background:'+ theme +'; '+ (options.type === 'vertical' ? 'height' : 'width') +':'+ scale +';'+ (options.type === 'vertical' ? 'bottom' : 'left') +':'+ (scaleFir || 0) +';"></div><div class="baiyun-slider-wrap" style="'+ (options.type === 'vertical' ? 'bottom' : 'left') +':'+ (scaleFir || scale) +';">' +
    '<div class="baiyun-slider-wrap-btn" style="border: 2px solid '+ theme +';"></div></div>'+ (options.range ? '<div class="baiyun-slider-wrap" style="'+ (options.type === 'vertical' ? 'bottom' : 'left') +':'+ scaleSec +';"><div class="baiyun-slider-wrap-btn" style="border: 2px solid '+ theme +';"></div></div>' : '') +'</div>';
    ...
    if(options.showstep){
      ...
      for(var i = 1; i < number + 1; i++) {
        ...
        if(step < 100){
          item += '<div class="baiyun-slider-step" style="'+ (options.type === 'vertical' ? 'bottom' : 'left') +':'+ step +'%"></div>'
        }
      };
    ...
    if(options.input && !options.range){
      var elemInput = $('<div class="baiyun-slider-input baiyun-input"><div class="baiyun-slider-input-txt"><input type="text" class="baiyun-input"></div><div class="baiyun-slider-input-btn"><i class="baiyun-icon baiyun-icon-up"></i><i class="baiyun-icon baiyun-icon-down"></i></div></div>');
  ...
  Class.prototype.slide = function(setValue, value, i){
    ...
    //拖拽元素
    ,elemMove = $(['<div class="baiyun-auxiliar-moving" id="LAY-slider-moving"></div'].join(''))
