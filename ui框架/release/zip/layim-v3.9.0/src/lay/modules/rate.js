...
baiyun.define('jquery',function(exports){
  ...
  ,rate = {
    config: {}
    ,index: baiyun.rate ? (baiyun.rate.index + 10000) : 0
  ...
  //字符常量
  ,MOD_NAME = 'rate',ELEM_VIEW = 'baiyun-rate', ICON_RATE = 'baiyun-icon-rate', ICON_RATE_SOLID = 'baiyun-icon-rate-solid', ICON_RATE_HALF = 'baiyun-icon-rate-half'
  
  ,ICON_SOLID_HALF = 'baiyun-icon-rate-solid baiyun-icon-rate-half',  ICON_SOLID_RATE = 'baiyun-icon-rate-solid baiyun-icon-rate',  ICON_HALF_RATE = 'baiyun-icon-rate baiyun-icon-rate-half'
  ...
  Class.prototype.render = function(){
    ...
    //组件模板
    var temp = '<ul class="baiyun-rate" '+ (options.readonly ? 'readonly' : '') +'>';
    for(var i = 1;i <= options.length;i++){
      var item = '<li class="baiyun-inline"><i class="baiyun-icon '
      ...
      if(options.half){
        if(parseInt(options.value) !== options.value){
          if(i == Math.ceil(options.value)){
            temp = temp + '<li><i class="baiyun-icon baiyun-icon-rate-half" '+ style +'></i></li>';
          }else{
            temp = temp + item 
          } 
        }else{
    ...
    temp += '</ul>' + (options.text ? ('<span class="baiyun-inline">'+ options.value + '星') : '') + '</span>';
