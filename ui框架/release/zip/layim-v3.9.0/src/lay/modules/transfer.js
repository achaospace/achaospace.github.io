/**
 ...
 @Name：baiyun.transfer 穿梭框
 @Author：贤心
 @License：MIT

 */

baiyun.define(['laytpl', 'form'], function(exports){
  "use strict";
  ...
  //字符常量
  ,ELEM = 'baiyun-transfer', HIDE = 'baiyun-hide', DISABLED = 'baiyun-btn-disabled', NONE = 'baiyun-none'
  ,ELEM_BOX = 'baiyun-transfer-box', ELEM_HEADER = 'baiyun-transfer-header', ELEM_SEARCH = 'baiyun-transfer-search', ELEM_ACTIVE = 'baiyun-transfer-active', ELEM_DATA = 'baiyun-transfer-data'
  
  //穿梭框模板
  ,TPL_BOX = function(obj){
    obj = obj || {};
    return ['<div class="baiyun-transfer-box" data-index="'+ obj.index +'">'
      ,'<div class="baiyun-transfer-header">'
      ...
      ,'<div class="baiyun-transfer-search">'
        ,'<i class="baiyun-icon baiyun-icon-search"></i>'
        ,'<input type="input" class="baiyun-input" placeholder="关键词搜索">'
      ,'</div>'
      ,'{{# } }}'
      ,'<ul class="baiyun-transfer-data"></ul>'
    ,'</div>'].join('');
  }
  
  //主模板
  ,TPL_MAIN = ['<div class="baiyun-transfer baiyun-form baiyun-border-box" lay-filter="LAY-transfer-{{ d.index }}">'
    ,TPL_BOX({
      index: 0
      ,checkAllName: 'layTransferLeftCheckAll'
    })
    ,'<div class="baiyun-transfer-active">'
      ,'<button type="button" class="baiyun-btn baiyun-btn-sm baiyun-btn-primary baiyun-btn-disabled" data-index="0">'
        ,'<i class="baiyun-icon baiyun-icon-next"></i>'
      ,'</button>'
      ,'<button type="button" class="baiyun-btn baiyun-btn-sm baiyun-btn-primary baiyun-btn-disabled" data-index="1">'
        ,'<i class="baiyun-icon baiyun-icon-prev"></i>'
      ,'</button>'
    ,'</div>'
    ,TPL_BOX({
      index: 1
  ...
  Class.prototype.render = function(){
    ...
    that.layBtn = thisElem.find('.'+ ELEM_ACTIVE + ' .baiyun-btn');
  ...
  Class.prototype.noneView = function(thisDataElem, text){
    var createNoneElem = $('<p class="baiyun-none">'+ (text || '') +'</p>');
  ...
  Class.prototype.parseData = function(callback){
    ...
    baiyun.each(options.data, function(index, item){
      //解析格式
      ...
      baiyun.each(options.value, function(index2, item2){
        if(item2 == item.value){
          item.selected = true;
        }
      });
  ...
  Class.prototype.getData = function(value){
    ...
    ,options = that.config
    ,selectedData = [];
    baiyun.each(value || options.value, function(index, item){
      baiyun.each(options.data, function(index2, item2){
